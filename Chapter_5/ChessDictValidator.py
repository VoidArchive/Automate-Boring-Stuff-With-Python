# Dict needs to have exactly one black king and one white king
#  each player can only have at most 16 pieces, 8 pawns
#  valid space form '1a' to '8h' a piece can’t be on space '9z'.
...
def isValidChessBoard(board):
    ...
    for piece in all_pieces:
        ...
        if not lo <= cnt <= hi:   # Count needs to be between lo and hi
            if lo != hi:
                print(
                    f"There should between {lo} and {hi} {piece} but there are {cnt}")
            else:
